...
def createFormWindow():
    ...
    def raditiKNN(e):
        ...
        if (rastojanje.get()=="Euklidsko" or rastojanje.get()=="Manhattan" or rastojanje.get()=="Chebyshev"):
            DB.encodeParamValuesKNN()
        if (rastojanje.get()=="Euklidsko V2" or rastojanje.get()=="Manhattan V2" or rastojanje.get()=="Chebyshev V2"):
            DB.encodeParamValuesKNNV2()
        if (rastojanje.get()=="Hamming - Euklidsko"):
            DB.encodeParamValuesKNNHamming()

        KNN.classifyByKNN()

        klasa = KNN.KNN(rastojanje.get())
        procenjenaCena.config(text="korišćeno "+text+"rastojanje: \n"+klasa)
    ...
    rastojanje= ttk.Combobox(root, value=["Euklidsko","Euklidsko V2", "Manhattan", "Manhattan V2","Chebyshev","Chebyshev V2","Hamming - Euklidsko"])
