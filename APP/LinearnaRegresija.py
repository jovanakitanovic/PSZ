...
def linearnaRegresija(_W0,_W,_set):
    ...
    for num in range(0,1000):
        ...
        for trainCar in trainCars:
            L0+=float(trainCar["h"])-trainCar["cena"]+float(W0*lam) #lam=0, poslednji činilac =0
...
def proveraLinearneRegresije(W0,W,collection):
    ...
    for emlem in collection:
        ...
        percentage = abs(100-abs(float((emlem['h']-emlem['cena'])/emlem['cena']))*100)
    ...
    percentage = abs(100-abs(float((sumaH - sumaY) / sumaY)) * 100)
